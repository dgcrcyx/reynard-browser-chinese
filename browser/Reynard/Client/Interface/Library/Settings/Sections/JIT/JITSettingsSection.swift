...
final class JITSettingsSection: NSObject {
    ...
    private func confirmDDIDownload(for sender: UISwitch) {
        ...
        let alert = UIAlertController(
            title: "正在准备 JIT",
            message: "由于这是你第一次启用 JIT，Reynard 需要下载并挂载开发者磁盘镜像。这是 JIT 正常工作所必需的。",
    ...
    private func performanceDetailLabel() -> UILabel {
        ...
        detailLabel.text = "启用 JIT 可显著提升性能，并且是 WebAssembly 等功能所必需的。"
        return detailLabel
    }
    ...
    private func unsupportedVersionWarningLabel() -> UILabel {
        ...
        warningLabel.text = "这种基于配对的 JIT 启用方法在你当前使用的系统版本上无法正常工作。你可以在无 JIT 模式下使用浏览器；或者如果你的 iOS/iPadOS 版本支持 TrollStore，建议改用 TrollStore 版本的 IPA。"
        return warningLabel
    }
}
