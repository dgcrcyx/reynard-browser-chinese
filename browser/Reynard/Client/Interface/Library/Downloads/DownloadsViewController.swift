...
final class DownloadsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate, UIGestureRecognizerDelegate {
    ...
    private let emptyStateView = SidebarEmptyBackgroundView(message: "你下载的文件将显示在这里")
    ...
    fileprivate func makeDownloadsMenu() -> UIMenu {
        UIMenu(title: "", children: [
            UIAction(title: "打开下载文件夹", image: UIImage(named: "reynard.folder")) { [weak self] _ in
                self?.openDownloadsFolder()
            },
            UIAction(title: "清除下载历史", image: UIImage(named: "reynard.arrow.down.circle.badge.xmark")) { [weak self] _ in
    ...
    private func updateEmptyState() {
        emptyStateView.message = query.isEmpty ? "你下载的文件将显示在这里" : "没有找到匹配的下载记录"
    ...
    func tableView(
        ...
        case .completed:
            ...
            let openAction = UIContextualAction(style: .normal, title: "在\n文件中打开") { [weak self] _, _, completion in
                guard let self else {
                    completion(false)
                    return
                }
    ...
    private func confirmCancelDownload(
        ...
        AlertPresenter.show(
            title: "取消下载？",
            message: "你想停止下载 \(item.fileName) 吗？",
            buttons: [
